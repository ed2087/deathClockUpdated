...
class GetStories {
    ...
    async  getstoriesBYupvotesBYnumReadsBYcommentsBYlimit(userId, limit) {
        ...
        if (userId) {
            // Fetch the followed users for the current user
            ...
            // Get stories by followed users
            const followedStories = await Storys.aggregate([
                {
                    $match: {
                        isApproved: true, // Filter out stories that are not approved
                        owner: { $in: followedUserIds }, // Only stories from followed users
                    },
                },
                {
                    $project: {
            ...
            // Get additional stories if needed
            const remainingLimit = limit - followedStories.length;
            let additionalStories = [];
            if (remainingLimit > 0) {
                additionalStories = await Storys.aggregate([
                    {
                        $match: {
                            isApproved: true,
                            owner: { $nin: followedUserIds }, // Exclude stories from followed users
                        },
                    },
                    {
                        $project: {
            ...
            // Combine followed stories and additional stories
            combinedStories = [...followedStories, ...additionalStories];
        } else {
            // Get top stories without considering followed users
            combinedStories = await Storys.aggregate([
                {
                    $match: {
                        isApproved: true,
                    },
